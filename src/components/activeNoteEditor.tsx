...
const ActiveNoteEditor: React.FC<ActiveNoteEditorProps> = ({
  ...
  const collaboration = useNoteCollaboration(note._id, user);
  ...
  }, [collaboration.ready, note._id, setNotes, titleText]);

  useEffect(() => {
    const synchronizeLocalTitle = (event: Event) => {
      if (readOnly || !collaboration.ready) return;
  ...
  }, [collaboration.document, collaboration.ready, note._id, readOnly, titleText]);
  ...
  useEffect(() => {
    if (!collaboration.ready || note.type !== NoteType.Document) return;
    setNotes((previous) =>
      previous.map((existingNote) =>
        existingNote._id === note._id &&
  ...
  }, [collaboration.ready, content, note._id, note.type, setNotes]);
  ...
  }, [collaboration.status, collaboration.unsyncedChanges, onSaveStatusChange]);
  ...
  useEffect(() => {
    if (!collaboration.ready || note.type !== NoteType.Whiteboard) return;
      ...
      <div className="collaboration-loading" role="status">
        <strong>
          {collaboration.status === "error"
            ? "Unable to join this shared note"
            : "Joining the live note…"}
        </strong>
        <span>
          {collaboration.status === "error"
            ? "Check that your sharing permission is still active."
            : "Loading the conflict-free document state."}
        </span>
        {collaboration.status === "error" && (
